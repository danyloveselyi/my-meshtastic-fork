...
2. **Extended filesystem (272 KB, 68 pages)** - used **ONLY** for NodeDB:
   - `nodes.proto` - node database (can store 2,000-2,400 nodes)

## Benefits

- ✅ Main filesystem remains standard (28 KB) - **no changes to framework**
- ✅ NodeDB gets 272 KB (68 pages) - sufficient for 2,000-2,400 nodes
...
Extended filesystem is created **manually** via direct LittleFS API calls, so a custom fork with extended pages is **not required**. Standard framework is used for main filesystem.
...
### Extended filesystem (272 KB)
- **Address**: `0xA8000` (after application, limited by linker script)
- **Size**: 68 pages × 4 KB = 272 KB
- **Usage**: Only `nodes.proto`
- **Capacity**: 2,000-2,400 nodes (based on actual slot sizes: 69-82 bytes data + ~20-50 bytes LittleFS overhead = ~100-130 bytes per node)
- **Protection**: Ends at 0xEBFFF, Main FS starts at 0xED000 (4 KB gap) - prevents filesystem from overwriting Main FS

## Files

- `NodeDBExtendedFSImpl.h/cpp` - Core extended filesystem implementation (initialization, load/save protobuf)
- `NodeDBFilesystemAdapter.h/cpp` - Adapter for integrating extended FS with NodeDB (routing logic)
