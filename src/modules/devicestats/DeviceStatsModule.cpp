...
void DeviceStatsModule::formatDetailedMemoryStats(char* buffer, size_t bufferSize)
{
    ...
    #if defined(USE_EXTENDED_FS_FOR_NODEDB) && defined(RAK_4631)
    // Use getMainFSStats() for accurate calculation (same logic as extended FS)
    extern bool getMainFSStats(uint32_t* total, uint32_t* used, uint32_t* free);
    if (!getMainFSStats(&flashTotal, &flashUsed, &flashFree)) {
        // Fallback to estimated size if calculation fails
        flashTotal = 28 * 1024;  // LittleFS area size (7 pages * 4KB = 28 KB)
        flashUsed = 8 * 1024;  // 8 KB estimate
        flashFree = flashTotal - flashUsed;
    }
    #else
    // Standard NRF52: use estimated size
    flashTotal = 28 * 1024;  // LittleFS area size (7 pages * 4KB = 28 KB)
    flashUsed = 8 * 1024;  // 8 KB estimate
    flashFree = flashTotal - flashUsed;
    #endif
    ...
    bool memValid = true;
    // CRITICAL: Ensure flashTotal and heapTotal are initialized
    if (flashTotal == 0) {
        LOG_WARN(DS_LOG_PREFIX "flashTotal is 0, using fallback");
        flashTotal = 28 * 1024;  // Fallback to 28KB
        flashUsed = 0;
        flashFree = flashTotal;
        memValid = false;
    }
    if (heapTotal == 0) {
        LOG_WARN(DS_LOG_PREFIX "heapTotal is 0, using fallback");
        heapTotal = 200 * 1024;  // Fallback estimate
        heapFree = heapTotal / 2;
        memValid = false;
    }
    ...
    if (nodeDB) {
        ...
        if (NodeDBPersistentBackend::isAvailable()) {
            ...
            if (NodeDBPersistentBackend::isEnabled()) {
                ...
                storedNodes = NodeDBPersistentBackend::getTotalNodeCount();
                ...
                if ((millis() - backend_query_start) < MAX_BACKEND_QUERY_TIME_MS) {
                    cachedNodes = NodeDBPersistentBackend::getCachedNodeCount();
                } else {
                    LOG_WARN(DS_LOG_PREFIX "Backend query timeout after getTotalNodeCount, skipping cachedNodes");
                    cachedNodes = 0;
                }
                if ((millis() - backend_query_start) < MAX_BACKEND_QUERY_TIME_MS) {
                    freeFlashSlots = NodeDBPersistentBackend::getFreeFlashSlots();
                } else {
    ...
    // Use Main FS stats instead of Extended FS (Extended FS has been removed)
    extern bool getMainFSStats(uint32_t* total, uint32_t* used, uint32_t* free);
    nodeDBFSEnabled = getMainFSStats(&nodeDBFlashTotal, &nodeDBFlashUsed, &nodeDBFlashFree);
    #endif
