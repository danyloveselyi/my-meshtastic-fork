...
void DeviceStatsModule::sendAutoReply(const meshtastic_MeshPacket &original)
{
    ...
    if (isCommand && (commandEquals(trimmed, "mem") || commandEquals(trimmed, "memory"))) {
        ...
        float freeHeapKB = memGet.getFreeHeap() / 1024.0f;
        snprintf(replyBuffer, sizeof(replyBuffer),
    ...
    DS_LOG_INFO("DeviceStats command setmaxnodes apply value=%d from=0x%0x", maxNodes, original.from);
        ...
        if (!pinOk) {
            ...
            uint32_t currentNodes = nodeDB ? nodeDB->getNumValidMeshNodes() : 0;
...
void DeviceStatsModule::doPeriodicWork()
{
    ...
    if (monitoringNodeId != 0) {
        ...
        if (elapsed >= monitorIntervalMs) {
            ...
            if (freeHeap < MINIMUM_SAFE_FREE_HEAP + 2048) {
                ...
                if (router) {
                    int queueFree = router->getFromRadioQueueFree();
                    if (queueFree > 0) {
    ...
    if (memElapsed > 300000) { // 5 minutes = 300,000ms
        ...
        if (freeHeap < MINIMUM_SAFE_FREE_HEAP * 2) {
            ...
            if (nodeDB) {
                uint32_t nodeCount = nodeDB->getNumValidMeshNodes();
...
void DeviceStatsModule::formatMemoryStats(char* buffer, size_t bufferSize, const char* prefix)
{
    ...
    uint32_t flashUsed = memGet.getFlashUsed();  // This is firmware size, not "used" flash!
    uint32_t flashFree = (flashTotal > flashUsed) ? (flashTotal - flashUsed) : 0;  // Calculate correctly
    ...
    uint32_t storedNodes = nodeDB ? nodeDB->getNumValidMeshNodes() : 0;
...
void DeviceStatsModule::formatDetailedMemoryStats(char* buffer, size_t bufferSize)
{
    ...
    uint32_t flashTotal = memGet.getFlashTotal();
    uint32_t flashUsed = memGet.getFlashUsed();  // This is firmware size (~327KB)
    uint32_t flashFree = memGet.getFlashFree();   // Use correct method for free flash
    uint32_t heapTotal = memGet.getHeapSize();
    uint32_t heapFree = memGet.getFreeHeap();
    ...
    if (nodeDB) {
        ...
        storedNodes = nodeDB->getNumValidMeshNodes();
...
void DeviceStatsModule::formatNodesInfo(char* buffer, size_t bufferSize)
{
    ...
    if (nodeDB && timeValid) {
        ...
        uint32_t actualNodeCount = nodeDB->getNumValidMeshNodes();
